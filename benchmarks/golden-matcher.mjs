...
export const RULES = {
  ...
  '015[2] postMessage targetOrigin': { file: 'layouts/embed.html.erb',
    all: [/postMessage|targetOrigin|target origin/i, /dropped|scheme|host and port|origin rather|not an origin|full (referr?er )?URL|entire URL|includes? (the )?path|path component/i] },
  ...
  '028[0] missing double check': { file: 'webassets.go',
    all: [/double.?check|re-?check|check (the cache )?again|another (goroutine|caller)|already populated|populated by another|populated the cache|while (this call(er)? )?wait/i] },
  '028[1] error overwrites cache': { file: 'webassets.go',
    all: [/\berror\b|\berr\b|fetch (fails|failed)|failed (fetch|load)|unsuccessful/i, /cache|overwrit|entryPointAssetsCache/i] },
  ...
  '042[0] optimized negative offset': { file: 'paginator.py',
    all: [/OptimizedCursorPaginator|enable_advanced_features|advanced[- ]feature|advanced (negative|branch)/i, /negative|offset < 0/i] },
