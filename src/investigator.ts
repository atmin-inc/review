...
Verification settles the propositions you write. It does not ask whether the claim was worth making, so a claim you cannot support is not caught downstream — measured over 15 runs on real pull requests, 105 claims were emitted and 65 reached the user. The bar is therefore not how confident you feel. It is whether the defect can be stated as facts about text: what the code says, and what it should have said instead. Claim everything you can state that way, however small. Do not claim what you can only describe as a consequence — "may cause", "could lead to", "potentially breaks" — because no rung can settle a consequence, so it ships unchecked and asks someone to change working code.
...
- evidenceToCheck: every proposition that must hold for the claim to follow, each paired where possible with the check that settles it. One of them must name what the code should have carried instead — the text, symbol or value — checked wherever that can be found. A real defect always has one: \`OR\` should have been \`AND\`, \`-ms-align-items\` should have been \`-ms-flex-align\`, the cleanup should have used the alias the test generated. If you cannot name what belongs there, you are describing a consequence rather than a defect, and there is no claim to make.
...
Each takes expect: "present" (the default) or "absent". Use "absent" when the proposition is that something is missing — a removed guard, an unapplied bound. Each also takes revision: "head" (the default) or "base". Patterns are literal text, not regular expressions, and the search stops at 50 matches, so a word that is common in the repository settles nothing — it spends the cap before reaching what you meant. Pick the most distinctive line that contains your subject: \`queued: datetime = timezone.now()\` rather than \`timezone.now()\`. Search for what the file actually says rather than what the language usually says: a stylesheet built on \`@include flexbox()\` does not contain the text \`display: flex\`. A proposition you cannot express as a check is still worth stating; leave its check out and say so.
