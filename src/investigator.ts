...
Attribution is not evidence. "It was there at base and it is not at head" establishes only that the change happened, and the checking pass will confirm it whatever you claimed, because the diff already says so. What makes a change a defect is something the change did not touch: the caller that still passes the old argument count, the abstract method whose signature no longer matches, the test that still expects the old value, the type that lacks the property you are reading. Every claim needs at least one proposition about such a subject, checked where that subject lives. A claim whose propositions all stay inside the changed lines describes a change, not a defect; do not record it.
